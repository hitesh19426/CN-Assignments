...
MSMSI edges_cost;
MSMSPSS edges_interfaces;
vector<RoutingNode*> distanceVectorNodes;

void routingAlgo(vector<RoutingNode*> distanceVectorNodes, MSMSI &edges_cost, MSMSPSS &edges_interfaces);
...
int main() {
    ...
    */
    while(name != "EOE") { //End of entries
      for (int i=0; i<distanceVectorNodes.size(); i++) {
		...
		if(distanceVectorNodes[i]->getName() == name) {
			...
			cin>>myeth>>oeth>>cost>>oname;
			
			edges_cost[name][oname] = cost;
			edges_interfaces[name][oname] = NetInterface(myeth, oeth);

			for(int j=0; j<distanceVectorNodes.size(); j++){
				if(distanceVectorNodes[j]->getName() == name){
					distanceVectorNodes[j]->ips.push_back(myeth);
				}
			}
			...
			for(int j = 0 ; j < distanceVectorNodes.size(); j++) {
				if(distanceVectorNodes[j]->getName() == oname) {
					...
					*/
					distanceVectorNodes[i]->updateTblEntry(myeth, myeth, myeth, 0);
					distanceVectorNodes[i]->updateTblEntry(oeth, oeth, myeth, cost);
					break;
				}
			}
		}
    ...
    routingAlgo(distanceVectorNodes, edges_cost, edges_interfaces);
